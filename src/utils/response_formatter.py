...
class ResponseFormatter:
    ...
    def format_google_response(self, response: str) -> str:
        """Format Google response to extract the phase label."""
        try:
            print(f"Raw Google response: {response}")
            # Remove markdown formatting and clean up the text
            response = re.sub(r'\*\*|\*', '', response)
            response = response.strip()
    ...
    def format_xai_response(self, response: str) -> str:
        """Format XAI response to extract the phase label."""
        try:
            print(f"Raw XAI response: {response}")
            # Remove markdown formatting and clean up the text
            response = re.sub(r'\*\*|\*', '', response)
            response = response.strip()
